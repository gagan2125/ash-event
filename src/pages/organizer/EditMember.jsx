...
const EditMember = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch(`${url}/member/update-member/${id}`, {
                method: "PUT",
                headers: {
    ...
    useEffect(() => {
        const fetchMemberData = async () => {
            try {
                const response = await fetch(`${url}/member/get-member-by-id/${id}`);
                if (response.ok) {
            ...
            <div className="flex h-screen bg-black">
                ...
                <div className="flex-1 flex flex-col p-10 overflow-y-auto no-scrollbar">
                    <div className="flex justify-between items-center mb-4">
                        <h1 className="text-3xl font-semibold text-white">Edit Member</h1>
